...
onready var questionsArea = $VerticalContainer/QuestionsArea/PanelContainer/Questions
...
onready var tileHbox = $PopupPanel/TileHBox
...
func _question_start():
	if questions:
		...
		Client.send_data(["GR", playerName, correctQuestions, wrongQuestions, playerScore])
		pass
...
func prepareTiles():
	...
	var count = 0
	for tile in tiles2:
		tile.text = wordsInTiles[count]
		count = count + 1
	
	for tile in tiles + tiles2:
